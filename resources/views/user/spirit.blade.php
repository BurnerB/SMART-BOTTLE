...
{{--meta tags--}}
@section('meta')
    <title>{{ $spirit->name }}</title>
    <meta charset="utf-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge"/>
    <meta name="viewport" content="width=device-width,initial-scale=1.0,maximum-scale=1"/>
    <meta name="author" content="{{ $seo->author }}">
    <!-- description -->
    <meta name="title" content="{{ $spirit->name }}">
            ...
            <div class="row">
                ...
                <div class="col-sm-6 col-sm-push-1 matchHeight">
                    <section class="alignMiddle mobile-center">
                        ...
                        {!! htmlspecialchars_decode($spirit->body) !!}
                        <p class="price">ksh {{ $spirit->price }}</p>
                        <a href="https://wa.me/{{ $whatsapp->value }}?text={{ urlencode($spirit->text." of ".$spirit->name) }}" class="btn btn-default"><span>Order on whatsapp</span></a>
                    </section>
                </div>
            </div>
        </div>
