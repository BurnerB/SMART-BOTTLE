...
@section('main-content')
                ...
                <div class="col-sm-12">
                    <ul class="horz-menu">
                        @foreach($categories as $category)
                            <li class="{{ Request::is('spirits/'.$category->slug) ? 'active' : '' }}"><span><a href="{{ route('spirit_category',$category->slug) }}">{{ $category->name }}</a></span></li>
                        @endforeach
    ...
    <div class="container-fluid super-dark section no-padding">
        <div class="container">
            <div class="row">
                <div class="col-sm-12">
                    {{ $spirits->links('vendor.pagination.default') }}
                </div>
            </div>
        </div>
    </div>
